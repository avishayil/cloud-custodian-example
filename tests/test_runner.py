...
class CustodianPolicyTest(unittest.TestCase):
    ...
    def extract_test_policy(self, policy_file_path, policy_names: list):
        """Alter policies YAML, strip mode and notify sections."""
        with open(str(policy_file_path)) as f:
    ...
    def run_policy(self, policy_file_path, policy_names, dumps):
        ...
        # setup args for custodian to read in
        # Must have vars=None or you will recieve an attribute error
        # vars can be used to resolve policy variables if need be i.e. 'vars': {'my_policy_var': 'test'}
        args = {"output_dir": "output", "configs": test_policy_file_paths, "vars": None}
        config = Config.empty(**args)

        with patch("c7n.resources.iam.time.sleep", autospec=True):
            with patch(
                "c7n.policy.Policy.execution_mode", new_callable=PropertyMock
            ) as em:
                em.return_value = "pull"
                run(config)

        # compile resources. If there are multiple policies in the yaml config
        # each policy's result is an entry in the resources list.
